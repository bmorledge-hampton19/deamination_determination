...
for (i in seq_along(CPDMismatchData)) {
  plotPositionAcrossTimepointAndReadLength(CPDMismatchData[[i]]["1h"], includedTypes = c("C>T"),
                                           title = paste(c("CPD", titleCaseFiltering,
                                                           "C>T Mismatch Pos Freq", repNames[i]), collapse = ' '),
                                           zScoreTables = CPDMMFreqZScores[[i]])
  plotPositionAcrossTimepointAndReadLength(CPDMismatchData[[i]]["1h"], includedTypes = c("C>T"), posType = FIVE_PRIME,
                                           title = paste(c("CPD", titleCaseFiltering,
                                                           "C>T Mismatch Pos Freq", repNames[i]), collapse = ' '),
                                           zScoreTables = CPDMMFreqZScores[[i]])
}
...
for (i in seq_along(CPDGroupedStats)) {
  plotGroupedPositionStats(CPDGroupedStats[[i]]$three_prime[Timepoint == "1h"],
                           CPDGroupedStats[[i]]$five_prime[Timepoint == "1h"],
