---
title: "NHF1 General Read Sequence Figure Generation"
...
```{r CPD_sequence_frequencies_filtered, fig.width = 11, fig.height = 9}
minReadLength = 18
for (i in seq_along(CPDTGGSeqFreq)) {
  plotSequenceFrequencies(CPDTGGSeqFreq[[i]], title = paste("CPD TGG Frequencies", repNames[i]),
                          expansionOffset = expansionOffset, showThreePrimeCutSite = TRUE,
                          showFivePrimeCutSite = TRUE, querySequences = c("TGG"), minReadLength = minReadLength)
}

for (i in seq_along(CPDCGASeqFreq)) {
  plotSequenceFrequencies(CPDCGASeqFreq[[i]], title = paste("CPD CGA Frequencies", repNames[i]),
                          expansionOffset = expansionOffset, showThreePrimeCutSite = TRUE,
                          showFivePrimeCutSite = TRUE, querySequences = c("CGA"), minReadLength = minReadLength)
}
```


```{r CPD_sequence_frequencies_filtered_x2, fig.width = 5, fig.height = 9}
minReadLength = 18
for (i in seq_along(CPDTGGSeqFreq)) {
  plotSequenceFrequencies(CPDTGGSeqFreq[[i]]$`1h`, title = paste("CPD TGG Frequencies", repNames[i]),
                          expansionOffset = expansionOffset, showThreePrimeCutSite = TRUE,
                          showFivePrimeCutSite = TRUE, querySequences = c("TGG"), minReadLength = minReadLength)
}

for (i in seq_along(CPDCGASeqFreq)) {
  plotSequenceFrequencies(CPDCGASeqFreq[[i]]$`1h`, title = paste("CPD CGA Frequencies", repNames[i]),
                          expansionOffset = expansionOffset, showThreePrimeCutSite = TRUE,
                          showFivePrimeCutSite = TRUE, querySequences = c("CGA"), minReadLength = minReadLength)
}
```
